//点击查询按钮，弹出查询框
var search1=new multiSearch2($(".btn1"),"reportFS26/tcompcode",null,null);//公司代码
//控制表格高度的显示
var heights = $(window).height();
$(".ch-kp-twentySix1 .t_r_content").height((heights - 210) + "px");
$(".ch-kp-twentySix2 .t_r_content").height((heights - 210) + "px");
//点击收起按钮
$(".retract").off("click").on("click", function () {
    if ($(".container form").css("display") == "block") {
        ...
        $(this).text("展开  ∨");
        var heights = $(window).height();
        $(".ch-kp-twentySix1 .t_r_content").animate({
            "height": (heights - 170) + "px"
        });
        $(".ch-kp-twentySix2 .t_r_content").animate({
            "height": (heights - 170) + "px"
        });
    } else {
...
$("#submit").off("click").on("click", function () {
    ...
    if (txt1 == "" || txt2 == "") {
        ...
        $(".export").off("click").on("click", function () {
            ...
            downloadFile(reportJson, ctx + "/crrc/reportFS26/downloadFile");
        });
         $.ajax({
             url: ctx + "/crrc/reportFS26/result01",
             data: reportJson,
             dataType: "json",
             ...
             success: function (data) {
                 if ('300' === data.statusCode || '301' === data.statusCode) {
                     alert(data.message);
                     $(".Mask").hide();
                     $(".ch-kp-twentySix1").html('');
                     $(".ch-kp-twentySix2").html('');
                     return true;
                 }
                   ...
                   //formatNumber(val.year_plan,2,1)
                     $.each(data.list, function (key, val) {
                    	 str += "<tr>"
		                             ...
		                             + "<td class='txtmd' style='width:150px;'>" + val.txtmd+ "</td>"
		                             + "<td class='last_year_cost' style='width:130px;text-align:right;'>" + formatNumber(val.last_year_cost,2,1) + "</td>"
		                             + "<td class='year_plan' style='width:130px;text-align:right;'>" + formatNumber(val.year_plan,2,1) + "</td>"
		                             + "<td class='period_plan' style='width:130px;text-align:right;'>" + formatNumber(val.period_plan,2,1) + "</td>"
		                             + "<td class='period_cost' style='width:130px;text-align:right;'>" + formatNumber(val.period_cost,2,1) + "</td>"
        ...
        if($(this).val()=="二级明细"){
          ...
          //二级明细表
            ...
            if (txt1 == "" || txt2 == "") {
                ...
                if(flag){
                	$.ajax({
    	                url: ctx + "/crrc/reportFS26/report02",
    	                data: reportJson,
    	                dataType: "json",
    	                contentType: "application/json;charset=utf-8",
    	                type: "post",
    	                success: function (data) {
    	                    if ('300' === data.statusCode || '301' === data.statusCode) {
    	                        alert(data.message);
    	                        $(".Mask").hide();
    	                        $(".ch-kp-twentySix1 table").html('');
    	                        $(".ch-kp-twentySix2 table").html('');
    	                        return true;
    	                    }
    	                    if (data.list.length == 0) {
    	                        $(".notSearch").next("div").hide();
