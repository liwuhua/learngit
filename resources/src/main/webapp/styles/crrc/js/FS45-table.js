...
$(".ch-kp-fortyFive .t_r_content").height((heights - 270) + "px");
//点击收起按钮
$(".retract").off("click").on("click", function () {
    if ($(".container form").css("display") == "block") {
        ...
        $(this).text("展开  ∨");
        var heights = $(window).height();
        $(".ch-kp-fortyFive .t_r_content").animate({
...
$("#submit").off("click").on("click", function () {
	 ...
	 var txt2 = $("#endTime").val().replace(/-/g, "");//报告日期
	 var reportJson = {};
     if (!blog) {
    	 return;
     }
    ...
    $(".export").on("click", function () {
	     ...
	     downloadFile(reportJson, ctx + "/crrc/reportFS45/downloadFile");
    });
    $.ajax({
        url: ctx + "/crrc/reportFS45/result",
        data: reportJson,
        dataType: "json",
        ...
        success: function (data) {
            if ('300' === data.statusCode || '301' === data.statusCode) {
                alert(data.message);
                $(".Mask").hide();
                $(".ch-kp-fortyFive table:eq(1)").html('');
                return true;
            }
            if (data.matnrpullcompar.length == 0) {
                ...
                $.each(data.lackorder, function (k, v) {
                    str += "<tr>"
		                        ...
		                        + "<td width='130px' class='LACK_ORDERNUM'>" + v.LACK_ORDERNUM + "</td>"//缺件订单数
		                        + "<td width='130px' class='SUM_AUFNR'>" + v.SUM_AUFNR + "</td>"//总订单数
		                        + "<td class='RATE'>" + v.RATE + "</td>"//订单齐套率
	                        + "</tr>";
                });
                $(".ch-kp-fortyFive table:eq(1)").html(str);
                trBg($(".ch-kp-fortyFive table:eq(1)"))
            }
            $(".Mask").hide();
	     ...
	     if (b && fold >= $(".t_r_content")[0].scrollHeight) {
		     ...
		     $.ajax({
			     ...
			     success: function (data) {
				     ...
				     item = item + 20;
				     $(".ch-kp-fortyFive table:eq(1)").append(str);
				     trBg($(".ch-kp-fortyFive table:eq(1)"));
				     $(".Mask").hide();
				     //$(".sureBtns").click();
				     b = true;
