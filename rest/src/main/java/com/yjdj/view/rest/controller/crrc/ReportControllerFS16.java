...
/**
 * Created by yangzhijie on 16/11/10.
 */
@Controller
@RequestMapping("/crrc/reportFS16")
public class ReportControllerFS16 extends CrrcGenericController {
    ...
    private static final String FS16 = "crrc/reportPageFS16";

    @RequestMapping(method = RequestMethod.GET)
    public String reportFS16() {
        return FS16;
    }
    ...
    public String getReport(@RequestBody QueryBean queryBean) {
        try {
            Object resultJson = iReportServiceFS16.getReportJson(queryBean);
    ...
    public String downloadFile(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (resultJson != null && queryBean.isExport()) {
                ...
                result.write(request, response, "合并利润表.xlsx");
            }
        } catch (IOException e) {
            logger.error("IO 操作异常" + e.getMessage());
            return AjaxObject.newError("IO 操作异常").setCallbackType("").toString();
    ...
    String upload(MultipartHttpServletRequest request, HttpServletResponse response, Object resultJson) {
        ...
        try {
            ...
            resultJson = iReportServiceFS16.importData(filePath);
        } catch (JsonProcessingException e) {
            logger.error("Json 数据封装异常" + e.getMessage());
            return AjaxObject.newError("Json 数据封装异常").setCallbackType("").toString();
        } catch (DataAccessException e) {
